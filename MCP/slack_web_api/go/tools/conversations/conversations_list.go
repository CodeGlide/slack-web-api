...
func Conversations_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/conversations.list%s", cfg.BaseURL, queryString)
...
func CreateConversations_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_conversations_list",
		mcp.WithDescription("Lists all channels in a Slack team."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Conversations_listHandler(cfg),
	}
}
