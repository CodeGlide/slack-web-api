...
func Calls_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/calls.update", cfg.BaseURL)
...
func CreateCalls_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_calls_update",
		mcp.WithDescription("Updates information about a Call."),
		...
		mcp.WithString("join_url", mcp.Description("Input parameter: The URL required for a client to join the Call.")),
		mcp.WithString("title", mcp.Description("Input parameter: The name of the Call.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Calls_updateHandler(cfg),
	}
}
