...
func CreateAdmin_conversations_ekm_listoriginalconnectedchannelinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_conversations_ekm_listOriginalConnectedChannelInfo",
		mcp.WithDescription("List all disconnected channels—i.e., channels that were once connected to other workspaces and then disconnected—and the corresponding original channel IDs for key revocation with EKM."),
