...
func Users_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users.list%s", cfg.BaseURL, queryString)
...
func CreateUsers_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_list",
		mcp.WithDescription("Lists all users in a Slack team."),
		mcp.WithString("token", mcp.Description("Authentication token. Requires scope: `users:read`")),
		...
		mcp.WithBoolean("include_locale", mcp.Description("Set this to `true` to receive the locale for users. Defaults to `false`")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_listHandler(cfg),
	}
}
