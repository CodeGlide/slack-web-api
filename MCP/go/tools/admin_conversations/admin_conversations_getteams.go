...
func Admin_conversations_getteamsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin.conversations.getTeams%s", cfg.BaseURL, queryString)
...
func CreateAdmin_conversations_getteamsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_conversations_getTeams",
		mcp.WithDescription("Get all the workspaces a given public or private channel is connected to within this Enterprise org."),
		mcp.WithString("token", mcp.Required(), mcp.Description("Authentication token. Requires scope: `admin.conversations:read`")),
		mcp.WithString("channel_id", mcp.Required(), mcp.Description("The channel to determine connected workspaces within the organization for.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Admin_conversations_getteamsHandler(cfg),
	}
}
