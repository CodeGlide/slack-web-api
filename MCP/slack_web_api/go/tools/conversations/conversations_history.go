...
func Conversations_historyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/conversations.history%s", cfg.BaseURL, queryString)
...
func CreateConversations_historyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_conversations_history",
		mcp.WithDescription("Fetches a conversation's history of messages and events."),
		mcp.WithString("token", mcp.Description("Authentication token. Requires scope: `conversations:history`")),
		mcp.WithString("channel", mcp.Description("Conversation ID to fetch history for.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Conversations_historyHandler(cfg),
	}
}
