...
func Admin_emoji_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin.emoji.list%s", cfg.BaseURL, queryString)
...
func CreateAdmin_emoji_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_emoji_list",
		mcp.WithDescription("List emoji for an Enterprise Grid organization."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Admin_emoji_listHandler(cfg),
	}
}
